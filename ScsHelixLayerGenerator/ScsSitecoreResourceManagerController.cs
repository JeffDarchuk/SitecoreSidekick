...
using ScsSitecoreResourceManager.Models;
using System.Text;
...
{
	class ScsSitecoreResourceManagerController : ScsController
	{
		...
		{
			string file = Directory.EnumerateFiles(_registration.GetScsRegistration<ScsSitecoreResourceManagerRegistration>().GetDataDirectory() + "\\Templates").FirstOrDefault(x => x.EndsWith(Request.QueryString["template"]));
		...
		{
			using (var fileStream = System.IO.File.Create(_registration.GetScsRegistration<ScsSitecoreResourceManagerRegistration>().GetDataDirectory() + "\\Templates\\" + Request.Headers["X_FILENAME"]))
			{
		...
		public ActionResult RemoveTemplate(string template)
		{
			...
			System.IO.File.Delete(_registration.GetScsRegistration<ScsSitecoreResourceManagerRegistration>().GetDataDirectory()+$"\\Templates\\{template}");
			return Content("we're good");
		}
		[ActionName("hggettargets.scsvc")]
		public ActionResult GetTargets(string template)
		{
			var targets = _registration.GetScsRegistration<ScsSitecoreResourceManagerRegistration>().GetPropertiesWrapper(template).Targets;
