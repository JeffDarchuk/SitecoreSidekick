			...
			try
			{
				using (new SecurityDisabler())
				{
					if (Data != null)
					{
						...
						var localItem = Factory.GetDatabase(database, true)
							.DataManager.DataEngine.GetItem(new ID(itemId), LanguageManager.DefaultLanguage, Sitecore.Data.Version.Latest);
						Status = new List<Tuple<string, string>>();
						Compare = new Dictionary<string, List<Tuple<string, string>>>();

						if (localItem == null)
						{
							Status.Add(new Tuple<string, string>("cmmissing", "This content item does not exist on the local server."));
						}
						...
						foreach (var chk in itemData.SharedFields)
						{
							...
							Compare["shared"].Add(chk.BlobId != null
								...
								: new Tuple<string, string>(chk.NameHint,
									HtmlDiff.HtmlDiff.Execute(HttpUtility.HtmlEncode(localItem[new ID(chk.FieldId)].Replace("\r", "")),
										HttpUtility.HtmlEncode(chk.Value.Replace("\r", "")))));
						}
						foreach (
							var ver in
							itemData.Versions.GroupBy(x => x.Language.Name)
								.Select(x => x.OrderBy(o => o.VersionNumber).Last())
								.Union(itemData.UnversionedFields))
						{
							Item languageItem = localItem.Database.DataManager.DataEngine.GetItem(new ID(itemId),
								LanguageManager.GetLanguage(ver.Language.Name), Sitecore.Data.Version.Latest);
							foreach (var chk in ver.Fields)
							{
								...
								Compare[ver.Language.Name].Add(chk.BlobId != null
									? new Tuple<string, string>(chk.NameHint, "Blob value changed")
									: new Tuple<string, string>(chk.NameHint,
										HtmlDiff.HtmlDiff.Execute(HttpUtility.HtmlEncode(languageItem[new ID(chk.FieldId)].Replace("\r", "")),
											HttpUtility.HtmlEncode(chk.Value.Replace("\r", "")))));
							}
						}
						if (Compare.Any())
							Status.Add(new Tuple<string, string>("cmfieldchanged",
								"This content item exists on the local server, however the fields have different values."));
						else
							Status.Add(new Tuple<string, string>("cmequal",
								"This content item is equivalent on the source and target servers."));
						if (ChildChanged)
						...
						foreach (Item child in localItem.Children)
						{
							...
							var newnode = new CompareContentTreeNode(child, false)
							{
								Status = new List<Tuple<string, string>>
									{
										new Tuple<string, string>("cmextra", "This item does not exist on the remote server.")
									},
								MissingRemote = true
							};
							Nodes.Add(newnode);
